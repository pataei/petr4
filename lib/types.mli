...
and Argument : sig
  type pre_t  =
      Expression of { value: Expression.t } (** Expressions with a [value]. *)
    | KeyValue of { key: P4string.t;
                    value: Expression.t } (** Key-value pairs with a [key] and a [value]. *)
    | Missing (** Missing (written [_]). *)
